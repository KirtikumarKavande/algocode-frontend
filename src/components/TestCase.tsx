...
const TestCase = ({ testCaseResult }) => {
  ...
  const getLoadingText = () => {
    ...
    const index = Math.floor((Date.now() / 1000) % texts.length);
    return texts[index];
  };
  ...
  if (isEmpty) {
    return (
      <div className="flex flex-col items-center justify-center h-[calc(100vh-12rem)] space-y-4 ">
        ...
        <p className="text-lg text-gray-600 animate-pulse">{getLoadingText()}</p>
      </div>
    );
  }

  const { status, error, results, totalTestCases, passedTestCases } = testCaseResult.testCaseResult;
          ...
          >
            <div className={`p-4 flex items-center justify-between ${
              ...
              <div className="flex items-center space-x-2">
                ...
                <span className="font-medium text-black">Test Case {result.testCase}</span>
              </div>
              <div className="flex items-center space-x-2 text-gray-600">
                <Clock className="w-4 h-4" />
                <span className="text-sm">{result.executionTime.toFixed(2)}s</span>
              </div>
            </div>
            ...
            <div className="p-4 space-y-3 bg-white">
              ...
              <div>
                <div className="text-sm font-medium text-gray-600">Your Output</div>
                <div className={`mt-1 font-mono text-sm p-2 rounded text-black ${
