...
const ProblemList = () => {
  ...
  return (
    ...
    {problemLinks && problemLinks.map((problem:Problem) => (
      <li 
        key={problem._id} 
        className="group border-b border-gray-200 hover:bg-gray-800 pb-2 transition-colors"
      >
        <button
        ...
        >
          <div className="flex items-center space-x-4">
            <span className="font-medium text-base">{problem.title.length > 40 ? `${problem.title.slice(0, 40)}...` : problem.title}</span>
            <span 
              className={`
